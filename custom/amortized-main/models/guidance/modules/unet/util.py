...
class MemoryEfficientCrossAttention(nn.Module):
    def __init__(self, query_dim, max_bs=4096, context_dim=None, heads=8, dim_head=64, dropout=0.0, lora_rank=4):
        super().__init__()
        ...
        self.to_q = LoRALinear(query_dim, inner_dim, rank=lora_rank, dropout=dropout)
        self.to_k = LoRALinear(context_dim, inner_dim, rank=lora_rank, dropout=dropout)
        self.to_v = LoRALinear(context_dim, inner_dim, rank=lora_rank, dropout=dropout)
        self.to_out = nn.Sequential(
            LoRALinear(inner_dim, query_dim, rank=lora_rank, dropout=dropout), 
            nn.Dropout(dropout)
        )
...
class LoRALinear(nn.Module):
    def __init__(self, in_features, out_features, rank=4, dropout=0.0):
        super().__init__()
