...
class GaussianRenderer:
    def __init__(self, opt: Options):
        ...
        self.proj_matrix = torch.zeros(4, 4, dtype=torch.float32)#.cuda()
    ...
    def cal_proj_matrix(self, tan_half_fovx, tan_half_fovy, znear, zfar):
        ...
        #print("tan_half_fovx",tan_half_fovx)
    ...
    def render(self, gaussians,render_Ks,render_c2ws,bg_color=None, scale_modifier=1): #cam_view, cam_view_proj, cam_pos, bg_color=None, scale_modifier=1):
        # gaussians: [B, N, 14]
        # cam_view, cam_view_proj: [B, V, 4, 4]
        # cam_pos: [B, V, 3]

        device = gaussians.device
        B, V = render_Ks.shape[:2]
        ...
        for b in range(B):
            ...
            for v in range(V):
                ...
                cam_view = torch.inverse(c2w.float()).transpose(0, 1)
                cam_pos = - c2w[:3, 3]
            
                cam_view_proj = cam_view @  self.proj_matrix.to(opacity.device)

        
              
                view_matrix = cam_view.float()
                view_proj_matrix = cam_view_proj.float()
                #print("view_matrix:",view_matrix.shape,"view_proj_matrix:",view_proj_matrix.shape)
